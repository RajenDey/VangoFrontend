...
export const ComfyWidgets = {
	...
	ZIPUPLOAD(node, inputName, inputData, app) {
		const zipWidget = node.widgets.find((w) => w.name === "zip");
		let uploadWidget;

		var default_value = zipWidget.value;
		Object.defineProperty(zipWidget, "value", {
			set : function(value) {
				this._real_value = value;
			},
		...
		async function uploadFile(file, updateNode) {
			try {
				...
				if (resp.status === 200) {
					const data = await resp.json();
					// Add the file as an option and update the widget value
					if (!zipWidget.options.values.includes(data.name)) {
						zipWidget.options.values.push(data.name);
					}

					if (updateNode) {
						zipWidget.value = data.name;
					}
				} else {
					alert(resp.status + " - " + resp.statusText);
				}
		...
		// Create the button widget for selecting the files
		uploadWidget = node.addWidget("button", "choose file to upload", "zip", () => {
			fileInput.click();
		});
		uploadWidget.serialize = false;
